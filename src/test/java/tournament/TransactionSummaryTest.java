...
//	String sManualSaving = "Manual_Savings";
	...
	public void TS4_Test() throws Exception {
		...
		Commentary.log(LogStatus.INFO, "PayeeAmount before adding transaction-> "+sPayeeAmount_before);

		Double processedPayeeAmount_before = h.processBalanceAmount(sPayeeAmount_before.replaceAll("[^0-9.-]", ""));
		Commentary.log(LogStatus.INFO, "Processed Payee amount is: "+processedPayeeAmount_before);
		...
		Double processedPayeeAmount_after = h.processBalanceAmount(sPayeeAmount_after.replaceAll("[^0-9.-]", ""));
		Commentary.log(LogStatus.INFO, "Payee amount is now: "+processedPayeeAmount_after);
		
		Double d = Double.parseDouble(tRec.getAmount());

		int compare = Double.compare(processedPayeeAmount_before-d, processedPayeeAmount_after);
...
//		Double dPayeeAmount_afterAddingTransaction = h.processBalanceAmount(sPayeeAmount_afterAddingTransaction.replace("shop ", ""));
		Double processedPayeeAmount_afterAddingTransaction = h.processBalanceAmount(sPayeeAmount_afterAddingTransaction.replaceAll("[^0-9.-]", ""));

		Commentary.log(LogStatus.INFO, "Payee amount after adding an expense transaction is now: "+processedPayeeAmount_afterAddingTransaction);
...
//		Double dPayeeAmount_afterDeletion = h.processBalanceAmount(sPayeeAmount_afterDeletion.replace("shop ", ""));
		Double processedPayeeAmount_afterDeletion = h.processBalanceAmount(sPayeeAmount_afterDeletion.replaceAll("[^0-9.-]", ""));

		Commentary.log(LogStatus.INFO, "Payee amount after deletion is now: "+processedPayeeAmount_afterDeletion);

		Integer compare = Double.compare(processedPayeeAmount_before, processedPayeeAmount_afterDeletion);

		if (compare==0)
			Commentary.log(LogStatus.INFO, "PASS: Payee Tile amount initially in the Transaction Summary card was ["+processedPayeeAmount_before+"], added and then deleted the expense transaction for ["+tRec.getAmount()+"], as expected now the Payee Tile amount is seen same: ["+processedPayeeAmount_afterDeletion+"]");
		else
			Commentary.log(sa, LogStatus.FAIL, "FAIL: Payee Tile amount initially in the Transaction Summary card was ["+processedPayeeAmount_before+"], added and then deleted the expense transaction for ["+tRec.getAmount()+"], now the Payee Tile amount is seen as: ["+processedPayeeAmount_afterDeletion+"]");
...
//		Double dCategoryAmount_before = h.processBalanceAmount(sCategoryAmount_before.replace("Internet ", ""));
		Double processedCategoryAmount_before = h.processBalanceAmount(sCategoryAmount_before.replaceAll("[^0-9.-]", ""));
		Commentary.log(LogStatus.INFO, "Processed Category amount is: "+processedCategoryAmount_before);
...
//		Double dCategoryAmount_afterAddingTransaction = h.processBalanceAmount(sCategoryAmount_afterAddingTransaction.replace("Internet ", ""));
		Double processedCategoryAmount_afterAddingTransaction = h.processBalanceAmount(sCategoryAmount_afterAddingTransaction.replaceAll("[^0-9.-]", ""));

		Commentary.log(LogStatus.INFO, "Category tile amount after adding an expense transaction is now: "+processedCategoryAmount_afterAddingTransaction);
...
//		Double dCategoryAmount_afterDeletion = h.processBalanceAmount(sCategoryAmount_afterDeletion.replace("Internet ", ""));
		Double processedCategoryAmount_afterDeletion = h.processBalanceAmount(sCategoryAmount_afterDeletion.replaceAll("[^0-9.-]", ""));

		Commentary.log(LogStatus.INFO, "Category tile amount after deletion is now: "+processedCategoryAmount_afterDeletion);

		Integer compare = Double.compare(processedCategoryAmount_before, processedCategoryAmount_afterDeletion);

		if (compare==0)
			Commentary.log(LogStatus.INFO, "PASS: Category Tile amount initially in the Transaction Summary card was ["+processedCategoryAmount_before+"], added and then deleted the expense transaction for ["+tRec.getAmount()+"], as expected now the Category Tile amount is seen same: ["+processedCategoryAmount_afterDeletion+"]");
		else
			Commentary.log(sa, LogStatus.FAIL, "FAIL: Category Tile amount initially in the Transaction Summary card was ["+processedCategoryAmount_before+"], added and then deleted the expense transaction for ["+tRec.getAmount()+"], now the Category Tile amount is seen as: ["+processedCategoryAmount_afterDeletion+"]");
...
//		Double dCategoryAmount_afterEditing = h.processBalanceAmount(sCategoryAmount_afterEditing.replace("Internet ", ""));
		Double processedCategoryAmount_afterEditing = h.processBalanceAmount(sCategoryAmount_afterEditing.replaceAll("[^0-9.-]", ""));

		Double d = Double.parseDouble(tRec1.getAmount()); 

		Commentary.log(LogStatus.INFO, "Category tile amount after editing is now: "+processedCategoryAmount_afterEditing);

		Integer compare = Double.compare(processedCategoryAmount_before-d, processedCategoryAmount_afterEditing);
